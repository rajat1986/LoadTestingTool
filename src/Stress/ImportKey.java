/***********************************ImportKey Class *************************************\\
1. Provide Secure Channel Functionality
\\******************************************************************************************/
...
public class ImportKey {
	 ...
	 *            Alias for the key.
	 **/
	ImportKey(String keyfile, String certfile, String hsmcertfile,
			String defaultalias) {
		...
		// change this if you want another keystorefile by default
		String keystorename_hsm = System.getProperty("keystore");

		if (keystorename_hsm == null)
			keystorename_hsm = System.getProperty("user.home")
					+ System.getProperty("file.separator")
					+ "keystore_hsm.ImportKey"; // especially this ;-)

		// parsing command line input
		/*
		 * String keyfile = ""; String certfile = ""; if (args.length < 2 ||
		 * args.length>3) {
		 * System.out.println("Usage: java comu.ImportKey keyfile certfile [alias]"
		 * ); System.exit(0); } else { keyfile = args[0]; certfile = args[1]; if
		 * (args.length>2) defaultalias = args[2]; }
		 */

		try {
			// initializing and clearing keystore
			ks_host = KeyStore.getInstance("JKS", "SUN");
			ks_host.load(null, keypass.toCharArray());
			// System.out.println("Using keystore-file : "+keystorename);
			ks_host.store(new FileOutputStream(keystorename),
					keypass.toCharArray());
			ks_host.load(new FileInputStream(keystorename),
					keypass.toCharArray());
			...
			// storing host keystore
			ks_host.setKeyEntry(defaultalias, ff, keypass.toCharArray(), certs);
			...
			// initializing and clearing keystore
			ts_hsm = KeyStore.getInstance("JKS", "SUN");
			ts_hsm.load(null, keypass.toCharArray());
			// System.out.println("Using HSM keystore-file : "+keystorename_hsm);
			ts_hsm.store(new FileOutputStream(keystorename_hsm),
					keypass.toCharArray());
			ts_hsm.load(new FileInputStream(keystorename_hsm),
					keypass.toCharArray());

			// loading HSM CertificateChain
			CertificateFactory cfhsm = CertificateFactory.getInstance("X.509");
			InputStream certstreamhsm = fullStream(hsmcertfile);

			Collection c_hsm = cfhsm.generateCertificates(certstreamhsm);
			Certificate[] certs_hsm = new Certificate[c_hsm.toArray().length];

			if (c_hsm.size() == 1) {
				certstreamhsm = fullStream(hsmcertfile);
				// System.out.println("One HSM certificate, no chain.");
				Certificate cert = cfhsm.generateCertificate(certstreamhsm);
				certs_hsm[0] = cert;
			} else {
				// System.out.println("HSM Certificate chain length: "+c_hsm.size());
				certs_hsm = (Certificate[]) c_hsm.toArray();
			}

			// storing host keystore
			ts_hsm.setKeyEntry(defaultalias, ff, keypass.toCharArray(),
					certs_hsm);
			// System.out.println ("HSM certificate stored.");
			// System.out.println ("Alias:"+defaultalias+"  Password:"+keypass);
			ts_hsm.store(new FileOutputStream(keystorename_hsm),
					keypass.toCharArray());
